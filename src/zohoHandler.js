...
export const handleInvoiceCreated = async (req, res) => {
    try {
        ...
        const body = req.body;
        const phone = getField(body, 'customer_phone', 'Customer_number', 'Customer_Phone');
        ...
        if (!phone) {
            console.error('Missing phone number in handleInvoiceCreated');
        ...
        const response = await sendTemplateMessage(phone, 'invoices_with_pdf', 'en_US', components);
